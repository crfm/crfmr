...
In this example we have a single Excel files that contains a number of sheets. The data is a full year of landing statistics by boat and trips from one of the regional fleets. The Excel file is on [the web](http://www.hafro.is/~einarhj/crfmr/data-raw/logbook.xlsx). It can be downloaded by either clicking on the link or by using the `download.file`-function:
```{r, eval = FALSE}
...
```{r}
fil <- "data-raw/logbook.xlsx"  # you need to change the file path
```
