...
Note if you have not installed and of the above packages, e.g. you get an error message when running:
```
library(ggmap)
```
saying:
...
```
Once done you of course need to run again:
```{r, eval = FALSE}
library(ggmap)
```
...
d2 <- read.csv("http://www.hafro.is/~einarhj/data/sau-crfm-country-catches.csv",
               stringsAsFactors = FALSE)
...
`r columns(2)`
```{r}
ggplot(d2, aes(year, total)) + geom_line()
ggplot(d2, aes(year, total)) + geom_line() + expand_limits(y = 0)
```
...
Controlling  which values appear as tick marks
`r columns(3)`
```{r}
...
The reslution of "world" is not very high as can be seen if only want Barbados:
```{r}
...
`r columns(1, 1, 1.2)`
```{r, echo = FALSE}
m2 + geom_point(data = d, aes(lon, lat, colour = maturity))
```
