...
library(readxl)
d <- read_excel("data-raw/sau-crfm-country-catches.xlsx")
glimpse(d)
```
...
```{r}
excel_sheets("data-raw/sau-crfm-country-catches.xlsx")
```

If NAs are represented by something other than blank cells, set the na argument by e.g.:
```{r, eval = FALSE}
read_excel("data-raw/sau-crfm-country-catches.xlsx", na = "NA")
```
