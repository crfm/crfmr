...
R is not a database software So one normally has to import the data from some other souces.
...
```{r, eval = FALSE}
d <- data_frame(weight = numeric(0),
                length = numeric(0))
d <- edit(d)
```

Here the `edit`-function opens up a very primitive data-entry window with the data then being stored in the obect __d__. But as said R is not a good data entry medium so this is rarely used for anything else than _ad hoc_ entries of data.
...
```
If the data file are in folder called __data__ within the working directory:
```{r, eval = FALSE}
...
```{r, eval = FALSE}
minke <- read.csv("http://www.hafro.is/~bthe/data/minke.csv")
```
...
After the data has been imported one frequently checks the data to see what was exported is as expected. Some common functions are: 
```
head(d)     # shows the top 6 lines of dat
tail(d)     # shows the last 6 lines of dat
dim(d)      # shows the num row and col of dat
names(d)    # gives the column names of dat
